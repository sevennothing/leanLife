 ...
 */
public abstract class EditorViewPanel extends AnchorPane {
    private final Logger logger = LoggerFactory.getLogger(EditorViewPanel.class);
    ...
    protected EditorViewPanel(ThreadService threadService,
    ...
    private void initializePreviewContextMenus() {
        ...
        stopRenderingItem.selectedProperty().addListener((observable, oldValue, newValue) -> {
            controller.editorController.stopRenderingProperty().setValue(newValue);
        });
    ...
    public void browse() {
        threadService.runActionLater(() -> {
            ...
            controller.editorController.browseInDesktop(documentURI);
        });
    }
        ...
        threadService.runActionLater(() -> {
            for (String jsPath : jsPaths) {
                String format = String.format("var scriptEl = document.createElement('script');\n" +
                        ...
                        "document.querySelector('body').appendChild(scriptEl);", controller.editorController.getPort(), jsPath);
    ...
    public void setOnSuccess(Runnable runnable) {
        threadService.runActionLater(() -> {
            getWindow().setMember("leanlife", controller.editorController);
