...
editor.commands.addCommand({
    ...
    exec: function (editor) {
        ...
        leanlife.cutCopy(editor.getCopyText());
...
//    exec: function (editor) {
//        leanlife.saveAndCloseCurrentTab();
//    },
//    readOnly: true
//});
editor.commands.addCommand({
...
editor.commands.addCommand({
    ...
    exec: function (editor) {
        //leanlife.pasteRaw();
        var text = leanlife.clipboardValue();
...
var editorMenu = {
    asciidoc: {
        ...
        addHyperLink: function () {
            ...
            var pasted = leanlife.clipboardValue();
            if (isURL(pasted)) {
...
editor.commands.addCommand({
    ...
    exec: function () {
        leanlife.getShortcutProvider().getProvider().addUnderline();
    },
    readOnly: true
});
editor.commands.addCommand({
    ...
    exec: function () {
        leanlife.getShortcutProvider().getProvider().addBold();
    },
    readOnly: true
});
editor.commands.addCommand({
    ...
    exec: function () {
        leanlife.getShortcutProvider().getProvider().addHeading();
    },
    readOnly: true
});
editor.commands.addCommand({
...
editor.commands.addCommand({
    ...
    exec: function () {
        leanlife.getShortcutProvider().getProvider().addItalic();
    },
    readOnly: true
});
editor.commands.addCommand({
...
editor.commands.addCommand({
    ...
    exec: function (editor, selection) {
        ...
        // img tab
        if (textRange == "img") {
            leanlife.getShortcutProvider().getProvider().addImage();
            return;
        }
        // book tab
        if (textRange == "book") { // source generator
            leanlife.getShortcutProvider().getProvider().addBookHeader();
            return;
        }
        // article tab
        if (textRange == "article") { // source generator
            leanlife.getShortcutProvider().getProvider().addArticleHeader();
            return;
        }
        // math tab
        if (textRange == "math") { // math block generator
            leanlife.getShortcutProvider().getProvider().addMathBlock();
            return;
        }
        // uml tab
        if (textRange == "uml") { // uml block generator
            leanlife.getShortcutProvider().getProvider().addUmlBlock();
            return;
        }
        // ditaa tab
        if (textRange == "ditaa") { // ditaa block generator
            leanlife.getShortcutProvider().getProvider().addDitaaBlock();
            return;
        }
        // tree tab
        if (textRange == "tree") { // tree block generator
            leanlife.getShortcutProvider().getProvider().addTreeBlock();
            return;
        }
        // quote tab
        if (textRange == "quote") { // quote block generator
            leanlife.getShortcutProvider().getProvider().addQuote();
            return;
        }
        // src tab
        if (textRange == "src") { // source generator
            leanlife.getShortcutProvider().getProvider().addCode("");
            return;
        }
        if (textRange == "show:worker") {
            leanlife.showWorkerPane();
            return;
        }
        ...
        if (Array.isArray(srcMatch)) {
            if (srcMatch.length == 3) {
                var lang = srcMatch[2];
                leanlife.getShortcutProvider().getProvider().addCode(lang);
                return;
            }
        }
//            "tbl3,2" tab
        ...
        if (Array.isArray(tableMatch))
            if (tableMatch.length == 4) { // table generator
                ...
                leanlife.getShortcutProvider().getProvider().addBasicTable(row, column);
                return;
            }
        editor.indent();
    },
